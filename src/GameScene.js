class GameScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image("card", "assets/images/cards/qc.jpeg");

    for (let cardNumber of config.cards) {
      this.load.image(
        `card${cardNumber}`,
        `assets/images/svg/card${cardNumber}.svg`
      );
    }
  ...
  createSounds() {
    ...
    this.sounds.theme.play({ volume: 0.5 });
  }

  createText() {
    this.timeoutText = this.add.text(880, 10, "", {
      font: "40px Arial",
      fill: "#fff",
    });
  }
  ...
  createDeveloperText() {
    this.developerText = this.add.text(710, 1035, "", {
      font: "25px Arial",
      fill: "#000",
    });

    this.developerText.setText("Developed by: Nijat Aliyev (@developer.nijat)");
  }
